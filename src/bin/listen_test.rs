...
struct Chunk { text: String, #[allow(dead_code)] para: usize, silent: bool }
...
fn main() -> anyhow::Result<()> {
    ...
    for (i, c) in chunks.iter().enumerate() {
        ...
        eprintln!(
            ...
            wav.len() as f64 / kokoro::SR as f64, dt,
            (wav.len() as f64 / kokoro::SR as f64) / dt.max(1e-9),
            ph.chars().count(),
            c.text.chars().take(48).collect::<String>()
        );
        if !out.is_empty() { out.extend_from_slice(&gap); }
        out.extend_from_slice(&wav);
    }
    let spec = hound::WavSpec { channels: 1, sample_rate: kokoro::SR,
        bits_per_sample: 16, sample_format: hound::SampleFormat::Int };
    let path = root.join("listen-test/kokoro_onnx_af_heart.wav");
    let mut w = hound::WavWriter::create(&path, spec)?;
    for s in kokoro::to_i16(&out) { w.write_sample(s)?; }
    w.finalize()?;
    println!("\nwrote {}", path.display());
    println!("chunks:        {}", chunks.iter().filter(|c| !c.silent).count());
    println!("audio:         {audio_s:.2}s (file {:.2}s with gaps)", out.len() as f64 / kokoro::SR as f64);
